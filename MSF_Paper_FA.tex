...
data was selected to test the approach because it has been well recognized
in the literature for the last several decades \TODO{what has been
  recognized??}.  Thereby facilitating the verification of the results of
...
and Interleukin genes (IL6, IL27, IL23) which serve as a subset \TODO{??}
of CD4+ T helper cells. IFNB1 and IFNA1 were both identified as two of the
...
identified with four main \TODO{(more than X genes)} sub-graphs consisting
of 38, 85, 148 and 167 genes. IFNA1 was identified as one of the sources in
...
and IFNA1 were identified as the two sources out of the possible sources
\TODO{??}.
...
\subsection*{Modulated sub-graphs at 6~hpi}
 ...
 data. This was also done for the DEG obtained for each run, a cutoff of
 FDR less than 0.05 was used to compare the retrieved DEG from noisy data
 ...
 recall rate in the edgeR based DGEA, between 84~\% (6~hpi) and 93~\%
...
Gene enrichment analysis was performed on genes in \texttt{MSF} identified
sub-graphs for each time-point using Reactome Analyze data
tool~\cite{Reactome}. Reactome's over-representation analysis tests whether
certain Reactome pathways are enriched by the list of genes submitted. For
comparison the DEG results from edgeR were filtered using a cut off of
adjusted p-value < 0.1. This subset of DEG list was used for gene
enrichment analysis. Enriched pathways were further filtered by confidence
score p-values, pathways having p-value less $<$ 0.05 for cut off DEG list
and $>$ 1$^{-10}$ for pathway enrichment in \texttt{MSF} identified
sub-graph genes. Out of 282 pathways dis-regulated in the two comparison
analysis with three time-points each, pathways that were present in at
least three time-points are shown in Fig~\ref{fig:msfvsreactome} \TODO{rephrase; not really clear how this filtering of pathways shown was done}.
...
glycoprotein (GP), triggers the activation of
cytokines~\cite{Olejnik}. This pathway was not identified when a p-value
cut off DEG list was analyzed for enrichment. Due to \texttt{MSF}
considering the complete DEG results, even the weak signal at the earliest
time-point was detected; for example Toll-like receptor signaling. \TODO{what is the point you want to say and state it clearly. this is to vague}
...
The Ebola infection RNA-seq dataset analyzed during the current study are
...
\begin{figure*}[p]
	...
	\fbox{
		\begin{minipage}{17 cm}
			...
			\caption{\TODO{make plot smaller and consider to add DEG vor 0.1, 0.05 and 0.01 as you did before} The percentage of DEG analysis genes
...
\begin{figure*}[p]
	...
	\includegraphics[width=1.0\linewidth]{MSFvsReactome}
	\caption{The heatmap shows Reactome pathway enrichment for genes in \texttt{MSF} identifed sub-graphs for the three time-points and the Reactome pathway enrichment for subset of DEG with adjusted p-value cut off of < 0.1. Green color shows the pathway being enriched and red color shows pathway not found as enriched. \TODO{this is not really a heatmap (no quantitative value encoded in the color but only qualitative value); you might want to include different p-value cutof for the DGE; in your set up you have to comapre column 1 with column 4; column 2 with column 5, and so on; that is suboptimal; the time points are not chronological ordered.}}
