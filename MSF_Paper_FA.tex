...
\begin{figure*}[ht!]
	...
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=10.0cm]{Git/MSF_Paper/GFBstep3}
		\caption{}
	\end{subfigure}
	\caption{Overview of \texttt{MSF} process showing the steps to identify the modulated sub-graphs. (a) showing a network of genes with green nodes as genes with \textit{p}-values $< $ 0.05, and blue nodes are genes with \textit{p}-values $>$ 0.05, (b) the genes circled red show the two initial modulated sub-graphs, sub-graph1 found one with M,K,C and the second modulated sub-graph2 S,V,T, (c) shows the modulated sub-graph1 being extended to M,K,C,A,D, (d) shows both the modulated sub=paths merged with the addition of 3 genes Z,W,P.}
\end{figure*}

\begin{figure*}
	\centering
	\includegraphics[width=1.7\linewidth]{Git/MSF_Paper/Merged_Network}
