...
Finder (\texttt{MSF}). \texttt{MSF} would help transform the
information obtained from DGEA into understandable knowledge of signal
transduction of genes. \texttt{MSF} is freely accessible on github.
...
signaling network. \texttt{MSF} does not use predefined gene sets in
functional units, it builds up the sub-graphs from scratch using the
whole cell signaling network from Reactome. The network has nodes
corresponding to genes and edges representing their directed
...
calculate the individual \textit{p}-value for a sub-graph. An overall
\textit{p}-value of a sub-graph will express the significance of all
genes in the sub-graph being modulated together.
...
following.The pipeline of identification of modulated sub-graphs from
a network by \texttt{MSF} is depicted in Fig (1).
\newline
...
the network. It tries to extend the sub-graph to the most significant
neighboring gene. A single combined \textit{p}-value is calculated for
...
accepted. Then the next most significant neighboring gene is added to
the sub-graph and the combined \textit{p}-value is calculated, if the
new combined \textit{p}-value of three genes is smaller than the
combined \textit{p}-value of the first two genes, the extended
sub-graph is accepted. This step is iteratively repeated until no
...
modulated in the whole network~(Fig.~1b).\newline
...
This is done by testing all possible extension by N genes (default two
genes) for all genes in the sub-graph. Again, this step is iteratively
...
together to one big modulated sub-graph~(Fig.~1d). This step is
...
to action. Genes in circular loops in the sub-graphs have no source or
sink.
...
distributed noise was added to the sample read counts. Then DGEA was
carried out on the disturbed data with the same parameters as for the
native data, followed by analysis with \texttt{MSF}. Using this
criteria, the goal was to spot almost identical modulated sub-graphs
in the real and noisy data. This procedure was carried out 100 times
...
the three time-points are shown in~(Fig.~2). We were very stringent in
adding noise to the counts, which could be seen by the DEG analysis
robustness. For the last time-point (48hpi) 84\% genes of the
sub-graphs from native data were retrieved, for 24hpi robustness was
about 79\%. Robustness was 71\% at 6hpi, this could be due to that the
...
less than 10 percent to get more sources than observed.
...
list generated by edgeR was used as an input for \texttt{MSF}. The
EBOV infection data has three time-points 6, 24 and 48 hour post
infection (hpi). At 6hpi, five modulated sub-graphs were
identified. Most of the genes part of the sub-graphs were chemokines
...
\texttt{MSF} were type~I interferon induced genes. At 24hpi nine
...
\subsection*{Modulated sub-graphs at 6hpi}
 ...
 Three main modulated sub-graphs identified by \texttt{MSF} at 6hpi
...
At 6hpi Cytokegg identified the sub-graph genes to be enriched in
...
at 6hpi showed toll-like receptor signaling being most significantly
...
\begin{figure*}[ht!]
	...
	\begin{subfigure}{.5\textwidth}
		...
		\caption{Step 3}
	\end{subfigure}
	\caption{Overview of \texttt{MSF} process showing 3 step toy
          example to identify the modulated sub-graphs. (a) Input:
          showing a network of genes with green nodes as genes with
          \textit{p}-values $< $ 0.05, and blue nodes are genes with
          \textit{p}-values $>$ 0.05, (b) Step 1: genes circled red
          show the two initial modulated sub-graphs identified,
          sub-graph 1 with genes M,K,C and sub-graph 2 with genes
          S,V,T, (c) Step 2: shows the modulated sub-graph 1 being
          extended to M,K,C,A,D while no extension in sub-graph 2 (d)
          Step 3: shows both the modulated sub-graphs merging together
          with the addition of 3 genes Z,W,P.}
			\end{minipage}
}
\end{figure*}
...
\begin{figure*}
	...
	\caption{Shows the three modulated sub-graphs identified by
          MSF at 6hpi, where nodes are genes and edges are the
          ...
          flow chart against each sub-graph.}
		\end{minipage}
}
\end{figure*}
